...
$(document).on('click', '.twitter-header-tab', function (e) {
    ...
    if (PressedTwitterTab !== CurrentTwitterTab) {
        ...
        if (PressedTwitterTab === "twitter-mentions") {
            $.post('http://coderc-phone/ClearMentions');
        }

        if (PressedTwitterTab == "twitter-home") {

            $.post('http://coderc-phone/GetTweets', JSON.stringify({}), function (Tweets) {
        ...
        if (PressedTwitterTab == "twitter-self") {
            $.post('http://coderc-phone/GetSelfTweets', JSON.stringify({}), function (selfdata) {
        ...
        $.post('http://coderc-phone/GetMentionedTweets', JSON.stringify({}), function (MentionedTweets) {
        ...
        $.post('http://coderc-phone/GetSelfTweets', JSON.stringify({}), function (selfTweets) {
...
QB.Phone.Notifications.LoadTweets = function (Tweets) {
    ...
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
        ...
        let html = `<div class="twitter-no-tweets"><p>You have not send tweet.</p></div>`
...
QB.Phone.Notifications.LoadSelfTweets = function (Tweets) {
    ...
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
        ...
        let html = `<div class="twitter-no-tweets"><p>You have no sent tweet</p></div>`
...
$(document).on('click', '.tweet-delete', function (e) {
    ...
    console.log(id)
    $.post('http://coderc-phone/DeleteTweet', JSON.stringify({
        id: id,
    }));

    $.post('http://coderc-phone/GetSelfTweets', JSON.stringify({}), function (tweets) {
...
$(document).on('click', '#send-tweet', function (e) {
    ...
    if (TweetMessage != "" || TweetUrl != "") {
        ...
        $.post('http://coderc-phone/PostNewTweet', JSON.stringify({
        ...
        $.post('http://coderc-phone/GetHashtags', JSON.stringify({}), function (Hashtags) {
...
$(document).on('click', '#send-photo', function (e) {
    ...
    $.post('http://coderc-phone/PostNewImage', JSON.stringify({}),
        function (url) {
...
$(document).on('click', '.hashtag-tag-text', function (e) {
    ...
    if (!HashtagOpen) {
        ...
        $.post('http://coderc-phone/GetHashtagMessages', JSON.stringify({
            hashtag: Hashtag
